...
export default async function DashboardPage() {
    ...
    const stats = [
        { label: 'Total Tickets', value: allTickets.length, icon: <ClipboardList className="w-6 h-6" />, color: 'text-blue-600', bg: 'bg-blue-50' },
        { label: 'Open', value: allTickets.filter(t => t.status === 'Open').length, icon: <Flame className="w-6 h-6" />, color: 'text-amber-600', bg: 'bg-amber-50' },
        { label: 'In Progress', value: allTickets.filter(t => t.status === 'In Progress').length, icon: <Bot className="w-6 h-6" />, color: 'text-purple-600', bg: 'bg-purple-50' },
        { label: 'Closed', value: allTickets.filter(t => t.status === 'Closed').length, icon: <CheckCircle2 className="w-6 h-6" />, color: 'text-green-600', bg: 'bg-green-50' },
    ]
